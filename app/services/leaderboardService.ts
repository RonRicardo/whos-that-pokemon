...
export async function testLeaderboardService() {
    console.log('🧪 Testing LeaderboardService...')
    
    try {
      // Test 1: Submit a test score with detailed error logging
      console.log('Test 1: Submitting test score...')
      
      // Let's test the player creation first
      console.log('Step 1a: Testing player creation...')
      const { data: playerData, error: playerError } = await supabase
        .from('players')
        .insert({ username: 'TestPlayer' })
        .select()
        .single()
      ...
      // Now test score submission with more logging
      const testScore = await LeaderboardService.submitScore({
      ...
      if (testScore) {
        console.log('✅ Score submitted successfully:', testScore)
      } else {
        console.log('❌ Score submission failed - check browser console for errors')
      }
